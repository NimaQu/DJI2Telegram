...
`audio_gain` 是客户端 → bridge → 模块方向的线性 PCM 幅值系数，允许 `0.0–1.0`，默认 `1.0` 不变，`0.0` 将发往模块的音频静音，`0.9` 将样本幅值降低 10%（不代表主观响度降低 10%）。修改配置后重启服务生效，无控制 API。

客户端发来的音频在进入模块播放队列前乘以该系数；模块 → bridge → 客户端方向不做 gain 处理，保持原始幅值。浏览器初始音频、iOS 通话及独立音频诊断的上行均使用相同系数，只处理一次，不修改采样率、帧长或模块内部增益。降低输出电平不能恢复已经削波的波形。
